...
\title[Measure Concentration]{Measure concentration \DIFaddbegin \DIFadd{and Minimality }\DIFaddend for \DIFdelbegin \DIFdel{compact }\DIFdelend \DIFaddbegin \DIFadd{Immersions }\DIFaddend and \DIFdelbegin \DIFdel{minimal submanifolds of }\DIFdelend \DIFaddbegin \DIFadd{submersions on }\DIFaddend the Sphere}
...
%Abstract
%%%%%%%%%%%%%%%%%%%%
\begin{abstract}\textcolor{red}{V3\DIFdelbegin \DIFdel{.4}\DIFdelend \DIFaddbegin \DIFadd{.5}\DIFaddend }
Inspired by the equatorial concentration of measure phenomenon in the sphere $\ese^n(1)$, a result which is deduced from the general, (and intrinsic), concentration of measure in the sphere, we describe in this paper an extrinsic equatorial concentration of measure satisfied by the closed, (compact without boundary), and minimal submanifolds $\Sigma^m$ isometrically immersed in the sphere $\ese^n(1)$.\DIFdelbegin \DIFdel{v3.}\DIFdelend \end{abstract}
...
%DIFDELCMD < \begin{corollary}[Measure concentration around the equator $E$]\label{equator}%%%
\DIFdel{\
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Given the }\DIFdelend \DIFaddbegin \DIFadd{given the }\DIFaddend $\epsilon$-strip $\Omega_\epsilon$ around the equator $E$, we have that
\begin{equation}\DIFdelbegin %DIFDELCMD < \label{eq3}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{eq2.5}
\DIFaddend \frac{{\rm vol}\left(\Omega_\epsilon\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}\geq 1-\sqrt{\frac{\pi}{2}}e^{-\epsilon^2\frac{n-1}{2}}
\end{equation}
 and hence, for $0<\epsilon<\frac{\pi}{2}$ we have
\begin{equation}\label{eq3.1}
\lim_{n\to \infty}\frac{{\rm vol}\left(\Omega_\epsilon\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}=\DIFdelbegin \DIFdel{1
}\DIFdelend \DIFaddbegin \DIFadd{1.
}\DIFaddend \end{equation}
\DIFdelbegin %DIFDELCMD < \end{corollary}
%DIFDELCMD < %%%
\DIFdelend 

This \DIFdelbegin \DIFdel{Corollary \ref{equator} }\DIFdelend \DIFaddbegin \DIFadd{property }\DIFaddend comes from the concentration of measure stated in Theorem \ref{measurecon} in the following way: let us denote as $\mathbb{S}^{n+}$ the half sphere centered at the north pole and  as $\mathbb{S}^{n-}$ the half sphere centered at the south pole. Then, the $\epsilon$-strip around the equator can be expressed, in terms of the $\epsilon$-fattenings $\mathbb{S}^{n+}_{\epsilon}$ and $\mathbb{S}^{n-}_{\epsilon}$ as the intersection, 
...
\DIFaddbegin \DIFadd{This property is also known as ``fat equator effect'' (see \mbox{%DIFAUXCMD
\cite{Bengtsson}}\hskip0pt%DIFAUXCMD
) 
and can be alternatively deduced by using the cosinus  of the distance function to at fixed point and the concentration around the mean of a Lipschitz function (see Corollary V.2 of \mbox{%DIFAUXCMD
\cite{MS} }\hskip0pt%DIFAUXCMD
) as follows
}

\begin{theorem}\label{teo:mitjana}
 \DIFadd{Given the $\epsilon$-strip $\Omega_\epsilon$ around the equator $E$,  there exists an absolute constant $\delta>0$ such that
...
\frac{{\rm vol}\left(\Omega_\epsilon\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}\geq 1-4e^{-\delta\bar\epsilon^2(n+1)}
 ...
 \DIFadd{where $\bar\epsilon=\sin(\epsilon)$ and hence,
 }\begin{equation}\DIFadd{\label{eq3.1}
\lim_{n\to \infty}\frac{{\rm vol}\left(\Omega_\epsilon\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}=1.
}\end{equation}   
\end{theorem}
\DIFadd{For the sake of completeness we will prove this theorem in the Appendix \ref{appendix}.
}
...
%DIFDELCMD < %%%
\DIFdelend \item \DIFaddbegin \DIFadd{For each $m\in \mathbb{N}_{>0}$ , the best option for $\delta$  in inequality \eqref{eq4} is attained when
}$$ 
\DIFadd{\delta=\frac{1}{2}\left(1-\frac{1}{ \bar \epsilon^2 (m+1)}\right),
 }$$
\DIFadd{and inequality  \eqref{eq4} becomes
}$$
    \DIFadd{\frac{{\rm vol}(x^{-1}(\Omega(p,\epsilon))}{{\rm vol}(\Sigma)}\geq 1-\bar \epsilon \cdot e^{-\bar\epsilon^2\frac{m+1}{2}}\sqrt{e(m+1)}.
    }$$
\DIFadd{In order to compare with \eqref{eq3} we can use 
$
\delta=\frac{15}{32}
$ and inequality \eqref{eq4} will be 
}$$\DIFadd{\frac{{\rm vol}\left(x^{-1}(\Omega(p,\epsilon))\right)}{{\rm vol}(\Sigma)}\geq  1- 4e^{-\frac{15}{32}\bar\epsilon^2(m+1)}.}$$
\DIFaddend But it is important to remark that inequality (\DIFdelbegin \DIFdel{\ref{eq3}) in Theorem \ref{equator} }\DIFdelend \DIFaddbegin \DIFadd{\ref{eq2.5}) }\DIFaddend is not the same nor \DIFdelbegin \DIFdel{is it }\DIFdelend \DIFaddbegin \DIFadd{it is  }\DIFaddend a consequence of inequality (\ref{eq4}) in Theorem \ref{mainInt} \DIFaddbegin \DIFadd{(and nor can  be derived directly from \eqref{eq3})}\DIFaddend . Note that, in addition to the fact that the subtrahend in inequalities (\DIFdelbegin \DIFdel{\ref{eq3}}\DIFdelend \DIFaddbegin \DIFadd{\ref{eq2.5}}\DIFaddend ) and (\ref{eq4}) is not the same, (even though both contain a negative power of the exponential function), the exponents $\epsilon$ and $\bar\epsilon$ are not the same either.  \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In fact, $\bar\epsilon=\sin\epsilon \in [0,1]$, where $\epsilon \in [0,\frac{\pi}{2}]$, so $\bar\epsilon \leq \epsilon$\DIFdelbegin \DIFdel{and therefore, when $\Sigma^m= \mathbb{S}^n(1)$ and hence $m=n$, we have that, for $n \geq 3$,
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
\DIFdel{1-\sqrt{\frac{\pi}{2}}e^{-\epsilon^2\frac{n-1}{2}} \geq 1- \sqrt{2}e^{-\bar\epsilon^2\frac{n+1}{4}}
...
%DIF > \end{equation*}
}\DIFaddend This difference in the exponents of the bounds comes from the different techniques used in the proof of the \DIFdelbegin \DIFdel{Corollary }\DIFdelend \DIFaddbegin \DIFadd{measure concentration in the sphere }\DIFaddend and our theorem.
...
$$A_\epsilon:=\left\{x\in \Sigma\, :\, {\rm dist}^{\Sigma}(x,A)<\epsilon\, \right\}\DIFaddbegin \DIFadd{.
}\DIFaddend $$
\DIFaddbegin \DIFadd{This problem is related with the problem of finding the first eigenvalue for the Laplacian of a compact minimal submanifold because by using Theorem 6.9 of \mbox{%DIFAUXCMD
\cite{MS} }\hskip0pt%DIFAUXCMD
for any domain $A\subset \Sigma$ with $\frac{{\rm vol}(A)}{{\rm vol}( \Sigma)}=a$ we have  
}\begin{equation}\DIFadd{\label{eq:Lambda}
   \frac{{\rm vol}\left(A_\epsilon\right)}{{\rm vol}( \Sigma)}\geq 1-(1-a^2)e^{-\epsilon\sqrt{\lambda_1}\ln(1+a)}, 
}\end{equation}
\DIFaddend 


\DIFaddbegin \DIFadd{where $\lambda_1(\Sigma)$ is the first (non-zero) eigenvalue for the Laplacian of $\Sigma$.  S.T Yau conjecture in problem 100 of \mbox{%DIFAUXCMD
\cite{YauMR0645728}  }\hskip0pt%DIFAUXCMD
that the first eigenvalue of a compact minimal embedded hypersurface $\Sigma\subset \mathbb{S}^{n+1}$ is given by the dimension of $\Sigma$, namely
}$$\DIFadd{\lambda(\Sigma)=n.}$$
\DIFadd{If the Yau's conjecture is true it would  imply, by using inequality \eqref{eq:Lambda} that for any domain $A\subset \Sigma\subset \mathbb{S}^{n+1}$  of a minimal $n$-dimensional hypersurface, with 
 $\frac{{\rm vol}(A)}{{\rm vol}( \Sigma)}\geq \frac{1}{2}$ there will be two constants $c_1,c_2\in \mathbb{R}_{>0}$ such that
 }\begin{equation}\DIFadd{\label{eq:yau1}
   \frac{{\rm vol}\left(A_\epsilon\right)}{{\rm vol}( \Sigma)}\geq 1-c_1e^{-c_2\epsilon\sqrt{n}}. 
}\end{equation}
\DIFadd{This decay rhythm in the dimension is a weaker version of our decay rate on the dimension given by \eqref{eq4} but it should be true for other domains than any extrinsic fattening of the equator.
}

\DIFadd{The Yau's conjecture remains unsolved but it is known to be true in several  cases: for instance for homogeneous hypersurfaces (see \mbox{%DIFAUXCMD
\cite{MR0742598}}\hskip0pt%DIFAUXCMD
, \mbox{%DIFAUXCMD
\cite{MR0814078}}\hskip0pt%DIFAUXCMD
) and for certain isoparametric minimal hypersurfaces (see \mbox{%DIFAUXCMD
\cite{MR3080491}}\hskip0pt%DIFAUXCMD
) .   Choi and Wang proved in \mbox{%DIFAUXCMD
\cite{Choi1983559} }\hskip0pt%DIFAUXCMD
that for compact, embedded  and minimal hypersurfaces $\Sigma\subset\mathbb{S}^{n+1}$ there exists the following lower bound in the first eigenvalue
}$$\DIFadd{\lambda(\Sigma)\geq \frac{n}{2}.}$$
\DIFadd{This lower bound is enough to state the concentration of measure provided by inequality \eqref{eq:yau1} . Observe  that this concentration phenomena would be attained only for compact, embedded and minimal hypersurfaces, in contrast to the results presented in this paper which are valid for any mimally immersed compat submanifold of any codimension. Observe moreover that the  rhythm  of concentration provided by inequality \eqref{eq:yau1}  is slower than the rhythm provided  by \eqref{eq4}.
}
...
\end{remark}
\DIFaddbegin \DIFadd{Theorem \ref{mainInt} is proved using that each of the $N+1$  coordinates functions of the immersion map  $x:\Sigma\to \mathbb{S}^N\subset \mathbb{R}^{N+1}$ , when $x$ is a minimal immersion, is an eigenfunction for the Laplacian of $\Sigma$  , namely, for anny $p\in \Sigma$
}$$
\DIFadd{\Delta_\Sigma x^i(p)=-}{\DIFadd{\rm dim}}\DIFadd{(\Sigma)x^i(p), \quad i=1,\cdots, N+1 
}$$
\DIFadd{and that the norm for the gradient of the extrinsic distance function  $q\in \Sigma\mapsto r_p(q)={\rm dist}^{\mathbb{S}^N}(p,x(q))$
is bounded from above by
}$$
\DIFadd{\Vert \nabla r_p\Vert \leq 1.
}$$
\DIFadd{But this two properties are fulfilled also for Riemannian subemrsions with minimal fibers because (see \mbox{%DIFAUXCMD
\cite{} }\hskip0pt%DIFAUXCMD
for instance) if we have a Riemannian submersion $\pi:\Sigma\to \mathbb{S}^n$ such that for every point $p\in \mathbb{S}^n$, the submanifold $\pi^{-1}(p)\subset \Sigma$ is minimally immersed in $\Sigma$, the Laplacian of any basic function commutes with the submersion, }\emph{\DIFadd{i.e.}}\DIFadd{,
}$$
\DIFadd{\Delta_\Sigma f\circ \pi(x)=\Delta_{\mathbb{S}^n}f(\pi(x))
}$$
\DIFadd{and hence ,  for each of the $n+1$  coordinates functions of the submersion map  $\pi:\Sigma\to \mathbb{S}^n\subset \mathbb{R}^{n+1}$   is an eigenfunction for the Laplacian of $\Sigma$  , namely, for any $p\in \Sigma$
}$$
\DIFadd{\Delta_\Sigma \pi^i(p)=-n\pi^i(p), \quad i=1,\cdots, n+1 
}$$
\DIFadd{and that the norm for the gradient of the basic distance function  $q\in \Sigma\mapsto r_p(q)={\rm dist}^{\mathbb{S}^n}(p,\pi(q))$
is given by $
\Vert \nabla r_p\Vert = 1
$.
}\DIFaddend 

\DIFaddbegin \begin{theorem}\label{TeoSub}
...
\DIFadd{Observe that in the case of submersions with minimal fibers to the sphere  there is no control for the value of the first eigenvalue of the Laplacian. 
...
\DIFadd{(p,q)\mapsto \pi(p,q)=p
}$$
\DIFadd{is a Riemannian submersion with minimal fibers, indeed totally geodesic ones, and for any $a\in (0,n]$ there is a value of $\delta$ such that
}$$
\DIFadd{\lambda_1(\Sigma)<a.
}$$
\DIFadd{Thence, in contrast to what happens for minimal hypersurfaces embedded in the sphere, there are no lower bounds for the first eigenvalue of a compact manifold which admits a Riemannian submersion to the sphere with minimal fibers.
}
...
\DIFdel{1\geq \frac{{\rm vol}(x^{-1}(\Omega(p,\epsilon)))}{{\rm vol}(\Sigma)}\geq 1- \sqrt{2}e^{-\bar\epsilon^2(m+1)/4}
}\end{displaymath}%DIFAUXCMD
...
where \DIFdelbegin \DIFdel{$\bar\epsilon=\sin\epsilon \in (0,1)$}\DIFdelend \DIFaddbegin \DIFadd{$\bar\epsilon=\sin\epsilon$}\DIFaddend .  Hence, for each $p \in \ese^n(1)$ and \DIFdelbegin \DIFdel{$\epsilon \in (0,\frac{\pi}{2}]$}\DIFdelend \DIFaddbegin \DIFadd{$\epsilon>0$}\DIFaddend ,
$$
\lim_{m\to\infty}\frac{{\rm vol}\left(x^{-1}(\Omega(p,\epsilon))\right)}{{\rm vol}(\Sigma)}=1
$$
\end{theorem}
...
\section{\DIFadd{Appendix: Proof of theorem \ref{teo:mitjana}}}\label{appendix}

\DIFadd{Our theorem \ref{teo:mitjana}  is a consequence of the concentration of the mean value of a Lipschitz function on the sphere. Given a point $p\in \mathbb{S}^n$ the function 
}\DIFaddend $$
\DIFdelbegin \DIFdel{\frac{{\rm vol}(x^{-1}(\Omega(p,\epsilon))}{{\rm vol}(\Sigma)}\geq 1-e^{-\bar\epsilon^2\frac{m+1}{2+2\delta}}\sqrt{1+\frac{1}{\delta}}.
    }\DIFdelend \DIFaddbegin \DIFadd{q\mapsto \cos(r_p(q))=}\langle \DIFadd{p, q}\rangle 
\DIFaddend $$
\DIFdelbegin \DIFdel{Theorem \ref{extconc2} is obtained therefore by the particular choose of $\delta=1$ . But for any $\delta\in (0,1)$  we will obtain an improved growth of $\frac{{\rm vol}(x^{-1}(\Omega(p,\epsilon))}{{\rm vol}(\Sigma)}$ with respect to  the dimension $m$. Moreover, for  $m$ large enough we can assume that
$\bar \epsilon^2>\frac{1}{m+1}$ (as in theorem \ref{extrinsicconc1}) and by choosing
    }\DIFdelend \DIFaddbegin \DIFadd{is Lipschitz function with Lipschitz constant $1$ because given two points  $q,s\in \mathbb{S}^n$ 
}

$$
\DIFadd{\begin{aligned}
    \left\Vert \cos(r_p(q))-\cos(r_p(s))\right\Vert=&\Vert \langle p, q\rangle-\langle p, s\rangle \Vert =\Vert \langle p, q- s\rangle \Vert\\
\leq& \Vert q-s\Vert={\rm dist}^{\mathbb{R}^{n+1}}(q,s)\\
\leq&  {\rm dist}^{\mathbb{S}^n}(p,q).
\end{aligned}
}$$
\DIFadd{Moreover since $\Delta_{\mathbb{S}^{n+1}}\cos(r_p)=-n\cos(r_p)$ by using the divergence theorem we conclude that
}$$
\DIFadd{\langle \cos(r_p)\rangle=\frac{\int_{\mathbb{S}^n}\cos(r_p)}{{\rm vol}(\mathbb{S}^n)}=0.
}$$
\DIFadd{Hence by using corollary V.2 of \mbox{%DIFAUXCMD
\cite{MS} }\hskip0pt%DIFAUXCMD
with $\bar\epsilon=\sin(\epsilon)$ 
}\DIFaddend $$
\DIFdelbegin \DIFdel{\delta}\DIFdelend \DIFaddbegin \DIFadd{1-\frac{{\rm vol}(\Omega(p,\epsilon))}{{\rm vol}(\mathbb{S}^n)}}\DIFaddend =\DIFdelbegin \DIFdel{\frac{1}{\bar\epsilon^2(m+1)-1},
    }\DIFdelend \DIFaddbegin \DIFadd{\frac{{\rm vol}\left(\{q\in \mathbb{S}^n\, : \, \vert \cos(r_p(q))\vert \geq \bar\epsilon \}\right)}{{\rm vol}(\mathbb{S}^n)}\geq 1-4e^{-\delta \bar\epsilon^2(n+1)}
}\DIFaddend $$
\DIFdelbegin \DIFdel{We would obtain the improved bound
}\begin{displaymath}
    \DIFdel{\frac{{\rm vol}(x^{-1}(\Omega(p,\epsilon))}{{\rm vol}(\Sigma)}\geq 1-\bar \epsilon \cdot e^{-\bar\epsilon^2\frac{m+1}{2}}\sqrt{e(m+1)}.
    }\end{displaymath}%DIFAUXCMD
%DIFDELCMD <     %%%
\DIFdel{This two bounds lead us to conjecture the following bound 
    }\begin{displaymath}
    \DIFdel{\frac{{\rm vol}(x^{-1}(\Omega(p,\epsilon))}{{\rm vol}(\Sigma)}\geq 1-C e^{-\epsilon^2\frac{m-1}{2}},
    }\end{displaymath}%DIFAUXCMD
%DIFDELCMD <     %%%
\DIFdel{for some absolute constant $C>0$, in agreement with the intrinsic case.
}%DIFDELCMD < \end{remark}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{and the theorem is proved.
}\DIFaddend %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\DIFaddbegin \providecommand{\noopsort}[1]{}\providecommand{\singleletter}[1]{#1}%DIF > 
%DIF > \begin{thebibliography}{1}

\bibitem{Bengtsson}
\DIFadd{Ingemar Bengtsson and Karol }{\DIFadd{\.Z}}\DIFadd{yczkowski.
}\newblock {\em \DIFadd{Geometry of quantum states}}\DIFadd{.
}\newblock \DIFadd{Cambridge University Press, Cambridge, 2006.
}\newblock \DIFadd{An introduction to quantum entanglement.
}

\bibitem{Choi1983559}
\DIFadd{Hyeong~In Choi and Ai~Nung Wang.
}\newblock \DIFadd{A first eigenvalue estimate for minimal hypersurfaces.
}\newblock {\em \DIFadd{Journal of Differential Geometry}}\DIFadd{, 18(3):559 – 562, 1983.
}



\DIFaddend \bibitem[MS]{MS} V. D. Milman and G. Schechtman,
...
\DIFaddbegin \bibitem{MR0814078}
\DIFadd{Motoko Kotani.
}\newblock \DIFadd{The first eigenvalue of homogeneous minimal hypersurfaces in a unit
  sphere }{\DIFadd{$S^{n+1}(1)$}}\DIFadd{.
}\newblock {\em \DIFadd{Tohoku Math. J. (2)}}\DIFadd{, 37(4):523--532, 1985.
}


\bibitem{MR0742598}
\DIFadd{Hideo Mut}{\DIFadd{o}}\DIFadd{, Yoshihiro Ohnita, and Hajime Urakawa.
}\newblock \DIFadd{Homogeneous minimal hypersurfaces in the unit spheres and the first
  eigenvalues of their }{\DIFadd{L}}\DIFadd{aplacian.
}\newblock {\em \DIFadd{Tohoku Math. J. (2)}}\DIFadd{, 36(2):253--267, 1984.
}

\DIFaddend \bibitem[R]{R} A. Ros,
...
\DIFadd{Shing~Tung Yau, editor.
}\newblock {\em \DIFadd{Seminar on }{\DIFadd{D}}\DIFadd{ifferential }{\DIFadd{G}}\DIFadd{eometry}}\DIFadd{, volume No. 102 of }{\em
  \DIFadd{Annals of Mathematics Studies}}\DIFadd{.
}\newblock \DIFadd{Princeton University Press, Princeton, NJ; University of Tokyo Press,
  Tokyo, 1982.
}\newblock \DIFadd{Papers presented at seminars held during the academic year
  1979--1980.
}\bibitem{MR3080491}
\DIFadd{Zizhou Tang and Wenjiao Yan.
}\newblock \DIFadd{Isoparametric foliation and }{\DIFadd{Y}}\DIFadd{au conjecture on the first eigenvalue.
}\newblock {\em \DIFadd{J. Differential Geom.}}\DIFadd{, 94(3):521--540, 2013.
}\DIFaddend 
