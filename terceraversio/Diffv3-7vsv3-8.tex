...
\DIFdelbegin \DIFdel{There is }\DIFdelend \DIFaddbegin \DIFadd{On the other hand, there is a }\DIFaddend concentration of measure in the dual geometric setting with respect the isometric immersions given by the Riemmanian submersions. \DIFdelbegin \DIFdel{A compact manifold with }\DIFdelend \DIFaddbegin \DIFadd{Namely, we shall prove a concentration of measure satisfied by the compact manifolds which }\DIFaddend admits a Riemannian submersion onto the sphere with minimal fibers\DIFaddbegin \DIFadd{. In this vein, such compact manifolds
 }\DIFaddend accumulates almost the whole measure on the  points which project to the strips of any width \DIFdelbegin \DIFdel{surounding the equator }\DIFdelend \DIFaddbegin \DIFadd{surrounding the equator of the sphere }\DIFaddend (see Theorem \ref{TeoSub}).
...
Let  $x: \Sigma^m \to \mathbb{S}^n(1)$ be an isometric and minimal immersion of the closed $m$-dimensional manifold $\Sigma$ in the sphere  $\mathbb{S}^n(1)$. Then, for any point $p \in \mathbb{S}^n(1)$ , any  $\epsilon \in (0,\pi/2)$ and any  absolute $\delta\in [0,\frac{1}{2})$\DIFaddbegin \DIFadd{, (in the sense that do not depends on $n$), }\DIFaddend we have 
...
We are going to finish this Introduction presenting a concentration of measure result of the fiber bundle projecting on the strip $\Omega(p, \epsilon)$ of a \DIFaddbegin \DIFadd{compact without boundary }\DIFaddend Riemannian submersion $\pi: \Sigma \rightarrow \ese^n(1)$ when this submersion is minimal, namely, when the (compact) fibers $\pi^{-1}(q) \subseteq \Sigma$ with $q \in \ese^n(1)$ are minimally immersed in $\Sigma$. This result, which can be considered a dual property of the extrinsic equatorial concentration of measure given by Theorem \ref{mainInt}, comes from both the structural duality of the equations that govern the geometry of the submersion with respect to the corresponding equations of the immersions and from the fact that the projection $\pi$ commutes with the Laplacians $\Delta^\Sigma$ and $\Delta^{\ese^n(1)}$ when the fibers of $\pi: \Sigma \rightarrow \ese^n(1)$ are minimal submanifolds of $\Sigma$.

\begin{theorem}\label{TeoSub}
Let  $\pi: \Sigma^m \to \mathbb{S}^n(1)$ be a Riemannian submersion with minimal fibers  of the closed $m$-dimensional manifold $\Sigma$ on to the sphere  $\mathbb{S}^n(1)$. Then, for any point $p \in \mathbb{S}^n(1)$ , any  $\epsilon \in (0,\pi/2)$ and any \DIFaddbegin \DIFadd{absolute }\DIFaddend $\delta\in [0,\frac{1}{2})$ we have 
...
\end{equation}
Hence, for each  \DIFdelbegin \DIFdel{$\epsilon$}\DIFdelend \DIFaddbegin \DIFadd{$\epsilon \in (0,\pi/2)$}\DIFaddend ,
\begin{equation}
...
\DIFaddbegin \DIFadd{We shall finish this Introduction with some general observations concerning the statements of Theorems \ref{mainInt} and \ref{TeoSub}.
}\bigskip


\DIFaddend \begin{remark}\label{remarkone}\DIFaddbegin \DIFadd{\
}\DIFaddend 



\begin{enumerate}
\item In contrast to the intrinsic approach, in \DIFdelbegin \DIFdel{this extrinsic case }\DIFdelend \DIFaddbegin \DIFadd{the extrinsic case given by Theorem \ref{mainInt}}\DIFaddend , the proof \DIFaddbegin \DIFadd{of inequality \eqref{eq4} and equality \eqref{eq5} }\DIFaddend is based on the divergence theorem, rather than on the isoperimetric inequality, (as \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{it is the case of the proof of }\DIFaddend inequality (\ref{eq2.5}) and equality (\ref{eq2.5.1}) \DIFaddbegin \DIFadd{in Theorem \ref{equator}}\DIFaddend , proved as a corollary of Theorem \ref{measurecon}) or rather than on probabilistic arguments using the Gaussian distribution, (as \DIFaddbegin \DIFadd{it is done }\DIFaddend in Theorem \ref{teo:mitjana}, deduced as a direct application of Theorem \ref{thB}).
\medskip
...
$$\lambda\DIFaddbegin \DIFadd{_1}\DIFaddend (\Sigma)\geq \frac{n}{2}.$$
This lower bound is enough to state the concentration of measure provided by inequality \eqref{eq:yau1} \DIFaddbegin \DIFadd{in this particular context, namely, when we consider compact, embedded  and minimal hypersurfaces $\Sigma\subset\mathbb{S}^{n+1}$}\DIFaddend . Observe  that this concentration phenomena would be attained for fattenings of domains of any kind in compact, embedded and minimal hypersurfaces of the sphere, in contrast to the results presented in this paper which would be valid for a specific type of equatorial domains in the otherwise broader context of minimal and compact immersions of any codimension in the sphere. Observe moreover that, as we have noted before, the  rate of decline in the concentration of measure provided by inequality \eqref{eq:yau1}  is slower than the rate of decrease provided  by  inequality \eqref{eq4}.
\item Note that, when the case of Riemannian submersions $\pi: \Sigma \rightarrow \ese^n(1)$ with minimal fibers is considered, and unlike what happens with minimal and embedded immersions of codimension 1 in the sphere, (see observation (\DIFdelbegin \DIFdel{8}\DIFdelend \DIFaddbegin \DIFadd{6}\DIFaddend ) in Remark \ref{remarkone}), the first eigenvalue of the Laplacian is not bounded from below in general. 
Let us take for instance the family of Riemannian manifolds \DIFdelbegin \DIFdel{$\Sigma_\delta:=(\mathbb{S}^n(1)\times \mathbb{S}^1, g_{\rm can}+\delta^2d\theta^2)$}\DIFdelend \DIFaddbegin \DIFadd{$\Sigma_\delta:=(\mathbb{S}^n(1)\times \mathbb{S}^1, g_{\mathbb{S}^{n}(1)}+\delta^2 g_{\mathbb{S}^{1}(1)})$}\DIFaddend , and the family of Riemannian submersions $\pi_\delta: \Sigma_\delta \rightarrow \ese^n(1)$ given by
$$
...
$$\Delta^\Sigma(f\circ \pi)(q)=\bigg(\big(\Delta^{\ese^n(1)}f\big)\circ\pi\bigg)(q)$$
  ...
  $$
  \Delta^\Sigma r_p=\Delta^\Sigma (d_p\circ \pi)=\DIFaddbegin \big(\DIFaddend \Delta^{\ese^n(1)}d_p\DIFaddbegin \big)\DIFaddend \circ \pi=(n-1)\frac{\cos(d_p)}{\sin(d_p)}\circ \pi=(n-1)\frac{\cos(r_p)}{\sin(r_p)}
  $$
...
\DIFadd{As in the case of immersions, where we obtain Corollary \ref{cortwo} for the compact case as a direct consequence of Theorem \ref{twopiecegeneral}, we have the following corresponding corollary for compact submersions deduced as a direct application of Theorem \ref{twopiecegeneralsub}:
}\begin{corollary}\label{cortwoSub}
	\DIFadd{Let $\pi:\Sigma^m \to \mathbb{S}^n(1)$ be a complete, minimal and compact Riemannian submersion, and let $p$ be any point of $\mathbb{S}^n(1)$.  Then
    }\begin{itemize}
...
In the above Section, we have proved in \DIFdelbegin \DIFdel{Corollary \ref{cortwo} that when }\DIFdelend \DIFaddbegin \DIFadd{Corollaries \ref{cortwo} and \ref{cortwoSub} that, when the immersion }\DIFaddend $x: \Sigma^m \to \mathbb{S}^n(1)$ is  compact and minimal, \DIFaddbegin \DIFadd{(and respectively, the submersion $\pi:\Sigma^m \to \mathbb{S}^n(1)$ is compact and minimal), }\DIFaddend then \emph{always} there are points of $x(\Sigma)$\DIFaddbegin \DIFadd{, (or $\pi(\Sigma)$), }\DIFaddend included in any equatorial strip, some in one part of the equator, some in the other part. The question now is to estimate the \lq\lq amount" of such points \DIFaddbegin \DIFadd{(}\DIFaddend in $x(\Sigma)$ \DIFaddbegin \DIFadd{or $\pi(\Sigma)$),  }\DIFaddend which are included in these equatorial strips. 
...
$$
\Vert \nabla\DIFaddbegin \DIFadd{^\Sigma }\DIFaddend r_p(x)\Vert =\Vert \nabla\DIFaddbegin \DIFadd{\Sigma }\DIFaddend \left(d_p\circ \pi\right)\Vert=1, \quad x\in \Sigma-\{\pi^{-1}(p)\}\cup \{\pi^{-1}(-p) \}
$$
...
$$
U'(t)=\int_{\partial D_t}\DIFdelbegin \DIFdel{\frac{1}{\Vert \nabla r_p\Vert}}\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{\Vert \nabla^\Sigma r_p\Vert}}\DIFaddend ={\rm vol}(\partial D_t)
$$
for any $t\in (0,\pi)$. \DIFdelbegin \DIFdel{When }\DIFdelend \DIFaddbegin \DIFadd{Now, }\DIFaddend we consider the function $E:\Sigma\to \mathbb{R}$ given by
$$
q\in \Sigma\mapsto E(q)=\int_0^{r_p(q)}\frac{V(t)}{V'(t)}dt
$$
where $V(t)={\rm vol}(B_t(p))$, and $V'(t)=\frac{d}{dt}V(t)={\rm vol }(\partial B_t(p))$. \DIFdelbegin \DIFdel{By using \eqref{eq:lapsub} can be proved }\DIFdelend \DIFaddbegin \DIFadd{As  $E=E(r_p)$ is a radial function, we have, using equation \eqref{eq:lapsub}, }\DIFaddend that
$$
\Delta^{\Sigma}E=1.
$$
