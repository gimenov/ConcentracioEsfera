...
\title[Measure Concentration]{Measure concentration and Minimality for Immersions and submersions on the Sphere}
...
Then, given the $\epsilon$-strip $\Omega_\epsilon$ around the equator $E$, we have that
...
This property is also known as ``fat equator effect'' (see \cite{Bengtsson}) 
and can be alternatively deduced by using the cosinus  of the distance function to at fixed point and the concentration around the mean of a Lipschitz function (see Corollary V.2 of \cite{MS} ) as follows

\begin{theorem}\label{teo:mitjana}
 Given the $\epsilon$-strip $\Omega_\epsilon$ around the equator $E$,  there exists an absolute constant $\delta>0$ such that
\begin{equation}\label{eq3}
\frac{{\rm vol}\left(\Omega_\epsilon\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}\geq 1-4e^{-\delta\bar\epsilon^2(n+1)}
\end{equation}
 where $\bar\epsilon=\sin(\epsilon)$ and hence,
 \begin{equation}\label{eq3.1}
\lim_{n\to \infty}\frac{{\rm vol}\left(\Omega_\epsilon\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}=1.
\end{equation}   
\end{theorem}
For the sake of completeness we will prove this theorem in the Appendix \ref{appendix}.
...
\item For each $m\in \mathbb{N}_{>0}$ , the best option for $\delta$  in inequality \eqref{eq4} is attained when
$$ 
\delta=\frac{1}{2}\left(1-\frac{1}{ \bar \epsilon^2 (m+1)}\right),
 $$
and inequality  \eqref{eq4} becomes
$$
    \frac{{\rm vol}(x^{-1}(\Omega(p,\epsilon))}{{\rm vol}(\Sigma)}\geq 1-\bar \epsilon \cdot e^{-\bar\epsilon^2\frac{m+1}{2}}\sqrt{e(m+1)}.
    $$
In order to compare with \eqref{eq3} we can use 
$
\delta=\frac{15}{32}
$ and inequality \eqref{eq4} will be 
$$\frac{{\rm vol}\left(x^{-1}(\Omega(p,\epsilon))\right)}{{\rm vol}(\Sigma)}\geq  1- 4e^{-\frac{15}{32}\bar\epsilon^2(m+1)}.$$
But it is important to remark that inequality (\ref{eq2.5}) is not the same nor it is  a consequence of inequality (\ref{eq4}) in Theorem \ref{mainInt} (and nor can  be derived directly from \eqref{eq3}). Note that, in addition to the fact that the subtrahend in inequalities (\ref{eq2.5}) and (\ref{eq4}) is not the same, (even though both contain a negative power of the exponential function), the exponents $\epsilon$ and $\bar\epsilon$ are not the same either.  In fact, $\bar\epsilon=\sin\epsilon \in [0,1]$, where $\epsilon \in [0,\frac{\pi}{2}]$, so $\bar\epsilon \leq \epsilon$.  
...
%\end{equation*}
This difference in the exponents of the bounds comes from the different techniques used in the proof of the measure concentration in the sphere and our theorem.
...
$$
This problem is related with the problem of finding the first eigenvalue for the Laplacian of a compact minimal submanifold because by using Theorem 6.9 of \cite{MS} for any domain $A\subset \Sigma$ with $\frac{{\rm vol}(A)}{{\rm vol}( \Sigma)}=a$ we have  
\begin{equation}\label{eq:Lambda}
   \frac{{\rm vol}\left(A_\epsilon\right)}{{\rm vol}( \Sigma)}\geq 1-(1-a^2)e^{-\epsilon\sqrt{\lambda_1}\ln(1+a)}, 
\end{equation}


where $\lambda_1(\Sigma)$ is the first (non-zero) eigenvalue for the Laplacian of $\Sigma$.  S.T Yau conjecture in problem 100 of \cite{YauMR0645728}  that the first eigenvalue of a compact minimal embedded hypersurface $\Sigma\subset \mathbb{S}^{n+1}$ is given by the dimension of $\Sigma$, namely
$$\lambda(\Sigma)=n.$$
If the Yau's conjecture is true it would  imply, by using inequality \eqref{eq:Lambda} that for any domain $A\subset \Sigma\subset \mathbb{S}^{n+1}$  of a minimal $n$-dimensional hypersurface, with 
...
\end{equation}
This decay rhythm in the dimension is a weaker version of our decay rate on the dimension given by \eqref{eq4} but it should be true for other domains than any extrinsic fattening of the equator.

The Yau's conjecture remains unsolved but it is known to be true in several  cases: for instance for homogeneous hypersurfaces (see \cite{MR0742598}, \cite{MR0814078}) and for certain isoparametric minimal hypersurfaces (see \cite{MR3080491}) .   Choi and Wang proved in \cite{Choi1983559} that for compact, embedded  and minimal hypersurfaces $\Sigma\subset\mathbb{S}^{n+1}$ there exists the following lower bound in the first eigenvalue
...
\end{enumerate}
\end{remark}
Theorem \ref{mainInt} is proved using that each of the $N+1$  coordinates functions of the immersion map  $x:\Sigma\to \mathbb{S}^N\subset \mathbb{R}^{N+1}$ , when $x$ is a minimal immersion, is an eigenfunction for the Laplacian of $\Sigma$  , namely, for anny $p\in \Sigma$
$$
...
\Vert \nabla r_p\Vert \leq 1.
$$
But this two properties are fulfilled also for Riemannian submersions with minimal fibers because (see \cite{} for instance) if we have a Riemannian submersion $\pi:\Sigma\to \mathbb{S}^n$ such that for every point $p\in \mathbb{S}^n$, the submanifold $\pi^{-1}(p)\subset \Sigma$ is minimally immersed in $\Sigma$, the Laplacian of any basic function commutes with the submersion, \emph{i.e.},
$$
\Delta_\Sigma f\circ \pi(x)=\Delta_{\mathbb{S}^n}f(\pi(x))
$$
...
$$
\Delta_\Sigma \pi^i(p)=-n\pi^i(p), \quad i=1,\cdots, n+1 
$$
...
Observe that in the case of submersions with minimal fibers to the sphere  there is no control for the value of the first eigenvalue of the Laplacian. 
...
(p,q)\mapsto \pi(p,q)=p
$$
is a Riemannian submersion with minimal fibers, indeed totally geodesic ones, and for any $a\in (0,n]$ there is a value of $\delta$ such that
$$
\lambda_1(\Sigma)<a.
$$
...
Our theorem \ref{teo:mitjana}  is a consequence of the concentration of the mean value of a Lipschitz function on the sphere. Given a point $p\in \mathbb{S}^n$ the function 
$$
q\mapsto \cos(r_p(q))=\langle p, q\rangle 
$$
is Lipschitz function with Lipschitz constant $1$ because given two points  $q,s\in \mathbb{S}^n$ 
...
\end{aligned}
$$
Moreover since $\Delta_{\mathbb{S}^{n+1}}\cos(r_p)=-n\cos(r_p)$ by using the divergence theorem we conclude that
$$
\langle \cos(r_p)\rangle=\frac{\int_{\mathbb{S}^n}\cos(r_p)}{{\rm vol}(\mathbb{S}^n)}=0.
$$
Hence by using corollary V.2 of \cite{MS} with $\bar\epsilon=\sin(\epsilon)$ 
$$
1-\frac{{\rm vol}(\Omega(p,\epsilon))}{{\rm vol}(\mathbb{S}^n)}=\frac{{\rm vol}\left(\{q\in \mathbb{S}^n\, : \, \vert \cos(r_p(q))\vert \geq \bar\epsilon \}\right)}{{\rm vol}(\mathbb{S}^n)}\geq 1-4e^{-\delta \bar\epsilon^2(n+1)}
$$
