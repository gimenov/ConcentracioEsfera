...
Inspired by the equatorial concentration of measure phenomenon in the sphere $\ese^n(1)$, a result which is deduced from the general, (and intrinsic), concentration of measure in the sphere\DIFdelbegin \DIFdel{result presented by V. Milman and G. Schechtman in the lecture notes \mbox{%DIFAUXCMD
...
The deep connection between the notions of measure of sets, (in its meaning of Riemannian volume), distance between points and dimension in a Riemannian manifold is illustrated by the concentration of \DIFdelbegin \DIFdel{intrinsic }\DIFdelend \DIFaddbegin \DIFadd{(intrinsic) }\DIFaddend measure phenomenon in the sphere \DIFaddbegin \DIFadd{$\ese^n(1)$, (that we consider in this paper equipped with a metric of constant sectional curvature $1$), }\DIFaddend as it was presented  by V. Milman and G. Schechtman in the lecture notes \cite{MS}.

\DIFdelbegin \DIFdel{In our opinion, an interesting question that can be raised from this result appears when considering it from an extrinsic point of view, that is, from the point of view of the submanifold theory. In this way, we are going to present a new notion of }%DIFDELCMD < \lq\lq %%%
\DIFdel{extrinsic equatorial measure concentration "  for submanifolds in the sphere, taking as a starting point the description of the intrinsic version of this phenomenon.  
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The way to do that , as we shall see in this introduction, is based in the following idea:  from the general  concentration of intrinsic }\DIFdelend \DIFaddbegin \DIFadd{From this concentration of }\DIFaddend measure phenomenon it can be deduced \DIFdelbegin \DIFdel{an intrinsic equatorial }\DIFdelend \DIFaddbegin \DIFadd{a particular result, namely, an intrinsic }{\em \DIFadd{equatorial}} \DIFaddend measure concentration property satisfied by the strips around the equators of the sphere, which can be roughly described by saying that for ``very large" dimension, almost all measure in the sphere concentrates in these strips surrounding its equators\DIFaddbegin \DIFadd{, independently of its width}\DIFaddend . 

\DIFdelbegin \DIFdel{Then, this property shall be used in its turn to introduce }\DIFdelend \DIFaddbegin \DIFadd{Now, an interesting question that can be raised from this last result appears when considering it from an extrinsic point of view, that is, from the point of view of the submanifold theory. 
We introduce at this point  }\DIFaddend the concept of \lq\lq \DIFdelbegin \DIFdel{extrinsic }\DIFdelend \DIFaddbegin {\em \DIFadd{extrinsic}} \DIFaddend equatorial measure concentration"  for submanifolds in the sphere\DIFdelbegin \DIFdel{above mentioned}\DIFdelend : we shall study \DIFaddbegin \DIFadd{then }\DIFaddend the relative volume of the intersection of a closed and minimal submanifold with the above mentioned strips around the equators of the ambient sphere, concluding, as in the intrinsic case, that for \lq\lq very large" dimension, almost all measure in the submanifold concentrates in their intersection with the strips \DIFdelbegin \DIFdel{surrounding its equators. 
}\DIFdelend \DIFaddbegin \DIFadd{of any width surrounding these equators. 
}\newpage
\DIFaddend 

In the lecture notes by V. Milman and G. Schechtman \cite{MS} we can find the following \DIFdelbegin \DIFdel{concentration of (intrinsic) measure phenomenon in the sphere $\mathbb{S}^n(1)$}\DIFdelend \DIFaddbegin \DIFadd{theorem}\DIFaddend :

\begin{theorem}[Measure Concentration in the Sphere]\label{measurecon}\DIFaddbegin \DIFadd{\
}

 \DIFaddend If $A\subset \mathbb{S}^n(1)$ is a domain with ${\rm vol }(A)\geq \frac{1}{2} {\rm vol}(\mathbb{S}^n(1))$ then, for all $\epsilon \in [0,\frac{\pi}{2}]$, the  $\epsilon$-fattening of $A$ satisfies the inequality
...
\DIFaddend Here, the $\epsilon$-fattening of $A$ is defined\DIFaddbegin \DIFadd{, using the intrinsic distance in the sphere $\ese^n(1)$, }\DIFaddend as
$$A_\epsilon:=\left\{x\in \mathbb{S}^n(1)\, :\, {\rm dist}^{\mathbb{S}^n(1)}(x,A)<\epsilon\, \right\}
$$
 \DIFaddbegin \end{remark}
  \DIFaddend 

 \DIFdelbegin \DIFdel{This concentration of measure in the sphere }\DIFdelend \DIFaddbegin \DIFadd{Theorem \ref{measurecon} }\DIFaddend is a consequence of the \DIFdelbegin \DIFdel{P. }\DIFdelend Levy's isoperimetric inequality \DIFdelbegin \DIFdel{,  which it is stated in \mbox{%DIFAUXCMD
\cite{MS} }\hskip0pt%DIFAUXCMD
as follows:
}%DIFDELCMD < 

%DIFDELCMD <  \begin{theorem}[Isoperimetric Inequality]\label{isopineq} %%%
\DIFdel{For each $0<a<{\rm vol}(\mathbb{S}^n(1))$ and $\epsilon >0$,
}\begin{displaymath}
\DIFdel{\min_{A\subset \mathbb{S}^n(1), {\rm vol}(A)=a}{\rm vol}(A_\epsilon)
}\end{displaymath}%DIFAUXCMD
\DIFdel{exists and it  is attained on the $\epsilon$-fattening $\left(B^{n,1}_{r(a)}\right)_\epsilon$ of any geodesic ball $B^{n,1}_{r(a)} \subset \mathbb{S}^n(1)$ such that
}\begin{displaymath}
{\DIFdel{\rm vol}}\DIFdel{\left(B^{n,1}_{r(a)}\right)=a.
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < \end{theorem}
%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{We are going to do some observations about how Theorem \ref{measurecon} comes from Theorem \ref{isopineq}}\DIFdelend \DIFaddbegin \DIFadd{in the sphere, (see \mbox{%DIFAUXCMD
\cite{MS}}\hskip0pt%DIFAUXCMD
)}\DIFaddend .
  To see the idea behind the proof of \DIFdelbegin \DIFdel{inequality (\ref{eq1})we proceed in the following way: }\DIFdelend \DIFaddbegin \DIFadd{equality (\ref{eq2}), (which is, properly speaking, the concentration of measure phenomenon), }\DIFaddend let us consider a  domain $A$ with ${\rm vol }(A)=a \geq \frac{1}{2} {\rm vol}(\mathbb{S}^n(1))$ and let us fix $\epsilon \in [0,\frac{\pi}{2}]$. Then,  applying \DIFdelbegin \DIFdel{Theorem \ref{isopineq}}\DIFdelend \DIFaddbegin \DIFadd{Levy's isoperimetric inequality}\DIFaddend , and having into account that, fixed $\epsilon \in [0,\frac{\pi}{2}]$, the $\epsilon$-fattening of a geodesic ball $B^{n,1}_{r}$ is the ball $(B^{n,1}_{r})_{\epsilon}=B^{n,1}_{r+\epsilon}$, for all $r \in [0,\pi/2-\epsilon)$, \DIFdelbegin \DIFdel{it is easy to obtain }\DIFdelend \DIFaddbegin \DIFadd{we have }\DIFaddend the inequality

\DIFdelbegin \begin{displaymath}{\rm vol}\DIFdel{(A_\epsilon) \geq }{\DIFdel{\rm vol}}\DIFdel{\left(\left(B^{n,1}_{r(a)}\right)_\epsilon\right)= }{\DIFdel{\rm vol}}\DIFdel{\left(B^{n,1}_{r(a)+\epsilon}\right)\geq  }{\DIFdel{\rm vol}}\DIFdel{\left(B^{n,1}_{\pi/2+\epsilon}\right),}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
%DIF > \begin{aligned}
\DIFadd{\frac{{\rm vol}\left(A_\epsilon\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}\geq  \frac{{\rm vol}\left(B^{n,1}_{\pi/2+\epsilon}\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}= \frac{\int _0^{\frac{\pi}{2}+\epsilon} \sin^n t dt }{\int _0^\pi \sin^n t dt   }.
%DIF > \end{aligned}
}\end{equation}\DIFaddend 
\DIFdelbegin \DIFdel{where $B^{n,1}_{r(a)} \subseteq \mathbb{S}^n(1)$ is the $r(a)$-geodesic ball (with $r(a) \geq \pi/2$), such that
}\begin{displaymath}
{\DIFdel{\rm vol}}\DIFdel{\left(B^{n,1}_{r(a)}\right)=a \geq \frac{1}{2} }{\DIFdel{\rm vol}}\DIFdel{\left(\mathbb{S}^n(1)\right)= }{\DIFdel{\rm vol}}\DIFdel{\left(B^{n,1}_{\pi/2}\right).
}\end{displaymath}%DIFAUXCMD
\DIFdel{We conclude therefore that
}\begin{displaymath}
...
To \DIFdelbegin \DIFdel{see how to }\DIFdelend obtain equality (\ref{eq2}) from this point, we remark that , given $\epsilon \in [0,\frac{\pi}{2}]$,  in the intervals $[0, \frac{\pi}{2}+\epsilon] \subseteq [0,\pi]$, the function $\sin t$ attains its maximum of $1$ in $t=\frac{\pi}{2}$, so, when $n \to \infty$, the function $\sin^n t$ has a ``peak'' at $\frac{\pi}{2}$ and is negligible out of this value. Hence, 
\DIFdelbegin \begin{displaymath}\DIFdel{\lim_{n \to \infty} \frac{\int _0^{\frac{\pi}{2}+\epsilon} (\sin t)^n dt }{\int _0^\pi (\sin t)^n dt   }=1}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
...
Now, we are going to focus on \DIFdelbegin \DIFdel{another }\DIFdelend \DIFaddbegin \DIFadd{a particular }\DIFaddend concentration of measure phenomenon \DIFaddbegin \DIFadd{in the sphere, that we call }{\em \DIFadd{equatorial}}\DIFaddend , what happens in this case around any totally geodesic equator $\mathbb{S}^{n-1}(1)$ of the sphere $ \mathbb{S}^{n}(1)$, and which can be deduced from the general measure concentration described above. \DIFaddbegin \DIFadd{We will take  it as  a starting point for our definition of the extrinsic measure concentration of a properly immersed submanifold in the sphere.
}\DIFaddend 
...
\DIFdel{\Omega_\epsilon:=\left\lbrace  (x_1,\ldots,x_{n+1})\in \mathbb{S}^{n}(1)\, :\, -\sin(\epsilon)<x_{n+1}<\sin(\epsilon)\right\rbrace.
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation*}
\DIFadd{\Omega_\epsilon:=\left\lbrace  (x_1,\ldots,x_{n+1})\in \mathbb{S}^{n}(1)\, :\, -\sin(\epsilon)<x_{n+1}<\sin(\epsilon)\right\rbrace
}\end{equation*}\DIFaddend 

\DIFaddbegin \noindent \DIFadd{where $\epsilon \in [0,\frac{\pi}{2}]$
}

\DIFadd{We must remark at this point that any equator in the sphere can be placed in the position of $E$ using the accurated rotation, (an isometry which preserves volume and distances), so the argument we are going to show applies in fact for any equator.
}


\DIFaddend \begin{center}
\begin{figure}\DIFaddbeginFL \label{fig:e-strip}
...
\DIFdelbeginFL %DIFDELCMD < \label{fig:e-strip}
%DIFDELCMD < %%%
...
\begin{corollary}[Measure concentration around the equator $E$]\label{equator}\DIFaddbegin \DIFadd{\
}

\DIFaddend Given the $\epsilon$-strip $\Omega_\epsilon$ around the equator $E$, we have that
\DIFdelbegin \begin{displaymath}
\DIFdel{\frac{{\rm vol}\left(\Omega_\epsilon\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}\geq 1-\sqrt{\frac{\pi}{2}}e^{-\epsilon^2\frac{n-1}{2}}
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}\DIFadd{\label{eq3}
\frac{{\rm vol}\left(\Omega_\epsilon\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}\geq 1-\sqrt{\frac{\pi}{2}}e^{-\epsilon^2\frac{n-1}{2}}
}\end{equation}\DIFaddend 
 and hence, for \DIFdelbegin \DIFdel{$0<\epsilon<1$ we have
}\begin{displaymath}
\DIFdel{\lim_{n\to \infty}\frac{{\rm vol}\left(\Omega_\epsilon\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}=1
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{$0<\epsilon<\frac{\pi}{2}$ we have
}\begin{equation}\DIFadd{\label{eq3.1}
\lim_{n\to \infty}\frac{{\rm vol}\left(\Omega_\epsilon\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}=1
}\end{equation}\DIFaddend 
\end{corollary}

This Corollary \ref{equator} comes from the concentration of measure stated in Theorem \ref{measurecon} in the following way: let us denote as $\mathbb{S}^{n+}$ the half sphere centered at the north pole and  as $\mathbb{S}^{n-}$ the half sphere centered at the south pole. Then, the $\epsilon$-strip around the equator can be expressed, in terms of the $\epsilon$\DIFdelbegin \DIFdel{-fattening }\DIFdelend \DIFaddbegin \DIFadd{-fattenings }\DIFaddend $\mathbb{S}^{n+}_{\epsilon}$ and $\mathbb{S}^{n-}_{\epsilon}$ as \DIFaddbegin \DIFadd{the intersection, 
}\DIFaddend $$\Omega_\epsilon=\mathbb{S}^{n+}_{\epsilon} \cap\mathbb{S}^{n-}_{\epsilon} $$
\DIFdelbegin \DIFdel{where the domains $A^+=\mathbb{S}^{n+}$ and $A^-= \mathbb{S}^{n-}$ concentrates measure in the Sphere in the sense of the above Theorem \ref{measurecon}. On the other hand, as ${\rm vol}(\mathbb{S}^{n+}_\epsilon)={\rm vol}(\mathbb{S}^{n-}_\epsilon)$, and by }\DIFdelend \DIFaddbegin 

%DIF > \begin{figure}
 %DIF >    \centering
  %DIF >   \includegraphics[scale=0.25]{positiva}\quad\includegraphics[scale=0.3]{negativa}
   %DIF >  \caption{The $\epsilon$-strip is the intersection between the $\epsilon$-fattening of the north hemisphere $\mathbb{S}^{+}_\epsilon$ and the $\epsilon$-fattening of the north hemisphere $\mathbb{S}^{-}_\epsilon$.}
   %DIF >  \label{fig:asintersection}
%DIF > \end{figure}

\noindent \DIFadd{Now, inequality (\ref{eq3}) follow }\DIFaddend applying Theorem \ref{measurecon} to the \DIFdelbegin \DIFdel{domain $A^+=\mathbb{S}^{n+}$, we have that }\begin{displaymath}
\DIFdel{\begin{aligned}
...
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[scale=0.25]{positiva}%%%
\DIFdelFL{\quad}%DIFDELCMD < \includegraphics[scale=0.3]{negativa}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The $\epsilon$-strip is the intersection between the $\epsilon$-fattening of the north hemisphere $\mathbb{S}^{+}_\epsilon$ and the $\epsilon$-fattening of the north hemisphere $\mathbb{S}^{-}_\epsilon$.}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:asintersection}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{$\epsilon$-fattenings $\mathbb{S}^{n+}_{\epsilon}$ and $\mathbb{S}^{n-}_{\epsilon}$ and using the fact that ${\rm vol}(\mathbb{S}^{n+}_\epsilon)={\rm vol}(\mathbb{S}^{n-}_\epsilon)$.}\DIFaddend The conclusion is that for  \lq\lq very large'' dimension,  almost all measure in the Sphere is concentrated surrounding  the equator. 
...
\begin{remark}
 ...
 \begin{figure}
     ...
     \caption{\DIFdelbeginFL \DIFdelFL{Caption}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Two orthogonal equatorial bands which intersects in two small antipodal squares}\DIFaddendFL }
...
We use this last \lq\lq equatorial" measure concentration to introduce the notion of  {\em extrinsic equatorial concentration of measure}  of \DIFdelbegin \DIFdel{the submanifold $\Sigma \subseteq \mathbb{S}^n(1)$. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{a submanifold in the sphere. }\DIFaddend Let us consider $x: \Sigma^m \to \mathbb{S}^n(1)$ be a complete isometric immersion of the  $m$-dimensional manifold $\Sigma^m$ in the sphere \DIFaddbegin \DIFadd{$\mathbb{S}^n(1)$}\DIFaddend . Any totally geodesic equator $\mathbb{S}^{n-1}(1)$ on a sphere  $\mathbb{S}^n(1)$ can be described in terms of the intrinsic distance of its points to a given fixed point $p$, (and, in this sense, the equator $E$ that has served above to exemplify the phenomenon of concentration of the equatorial measure, is the set of points that are distant from the north pole by a distance equal to $\frac{\pi}{2}$). This description is given in the following way: given a point $p \in \mathbb{S}^n(1)$, \emph{the equator of $\mathbb{S}^n(1)$ with respect to $p$} is defined as 
...
\begin{enumerate}
\item In \DIFaddbegin \DIFadd{contrast to the intrisic case, in }\DIFaddend this extrinsic case, the proof is based on the divergence theorem, \DIFdelbegin \DIFdel{(}\DIFdelend rather than on the isoperimetric inequality\DIFdelbegin \DIFdel{, as occurs in the intrinsic case)}\DIFdelend .

\item We can recover the {\em intrinsic} equatorial concentration of measure  \DIFaddbegin \DIFadd{(\ref{eq3.1}) }\DIFaddend as a corollary of the {\em extrinsic} equatorial  concentration of measure \DIFaddbegin \DIFadd{(\ref{eq5})}\DIFaddend , taking $\Sigma=\mathbb{S}^n(1)$. In fact, if \DIFdelbegin \DIFdel{$\Sigma=\mathbb{S}^n(1)$}\DIFdelend \DIFaddbegin \DIFadd{$\Sigma^m=\mathbb{S}^n(1)$}\DIFaddend , then we can use $x$ as the identity map  and hence, 
$$x^{-1}(\Omega(p,\epsilon))=x(\Sigma) \cap \Omega(p,\epsilon) =\Omega(p,\epsilon)$$
\DIFdelbegin \DIFdel{so
}\DIFdelend \DIFaddbegin 

\noindent \DIFadd{so, from equality (\ref{eq5}), we obtain
}\DIFaddend $$
\lim_{n\to\infty}\DIFdelbegin \DIFdel{\frac{{\rm vol}\left(x^{-1}(\Omega(p,\epsilon))\right)}{{\rm vol}(\mathbb{S}^n(1))}}\DIFdelend \DIFaddbegin \DIFadd{\frac{{\rm vol}\left(x^{-1}(\Omega(p,\epsilon))\right)}{{\rm vol}(\Sigma)}}\DIFaddend =\lim_{n\to\infty}\frac{{\rm vol}\left(\Omega(p,\epsilon)\right)}{{\rm vol}(\mathbb{S}^n(1))}=1
$$
\DIFaddbegin \noindent \DIFadd{Hence, we have an alternative way to prove the intrinsic equatorial concentration of measure in the sphere which is not based in the isoperimetric inequality.
}\DIFaddend 

\item \DIFaddbegin \DIFadd{But it is important to remark that inequality (\ref{eq3}) in Theorem \ref{equator} is not the same nor is it a consequence of inequality (\ref{eq4}) in Theorem \ref{mainInt}. Note that, in addition to the fact that the subtrahend in inequalities (\ref{eq3}) and (\ref{eq4}) is not the same, (even though both contain a negative power of the exponential function), the exponents $\epsilon$ and $\bar\epsilon$ are not the same either. 
}

\DIFadd{In fact, $\bar\epsilon=\sin\epsilon \in [0,1]$, where $\epsilon \in [0,\frac{\pi}{2}]$, so $\bar\epsilon \leq \epsilon$  and therefore, when $\Sigma^m= \mathbb{S}^n(1)$ and hence $m=n$, we have that, for $n \geq 3$,
}

\begin{equation*}
\DIFadd{1-\sqrt{\frac{\pi}{2}}e^{-\epsilon^2\frac{n-1}{2}} \geq 1- \sqrt{2}e^{-\bar\epsilon^2\frac{n+1}{4}}
}\end{equation*}

\DIFadd{This difference in the exponents of the bounds comes from the different techniques used in the proof of the Corollary and our theorem.
}

\item  \DIFaddend It should rest open the statement of an {\em intrinsic} concentration of measure for compact and minimal submanifolds of the sphere, namely, if $A \subseteq \Sigma$ is a domain with ${\rm vol}(A) \geq \frac{1}{2} {\rm vol}(\Sigma)$, then the $\epsilon$-fattening of $A$ in $\Sigma$ satisfies the inequality
\begin{equation*}
...
where $-p$ denotes the antipodal point of $p \in \mathbb{S}^{n}(1)$. It is also evident  that any equator in the sphere $\mathbb{S}^n(1)$ is an equator with respect to some $p$.
  ...
  Therefore,  a natural question which arises in this context is following: under what conditions, (topological, analytical, curvature restrictions, etc), it is guaranteed, (for any dimension of the ambient sphere and any co-dimension of the immersed submanifold), that the submanifold cuts all the equators of the ambient sphere, namely
        ...
        $$x(\Sigma) \cap \Omega(p,\epsilon) \neq \emptyset\DIFdelbegin \DIFdel{?}\DIFdelend , (\text{i.e.}, x^{-1}(\Omega(p,\epsilon))\neq \emptyset\,\,\,\forall p \in \mathbb{S}^n(1))$$

    Our first result  \DIFaddbegin \DIFadd{takes the form of a kind of a }\lq\lq \DIFadd{high dimensional two-piece property", and }\DIFaddend describes the position of the submanifold with respect the equators of $\mathbb{S}^n(1)$ and their strips, when \DIFdelbegin \DIFdel{the submanifold is connected and parabolicand takes the form of a kind of a high dimensional two-piece property}\DIFdelend \DIFaddbegin \DIFadd{this submanifold is minimal and parabolic}\DIFaddend :
...
\end{lemma}
\begin{proof}
  Applying Takahasi's Lemma, \DIFaddbegin \DIFadd{(see \mbox{%DIFAUXCMD
...
\begin{corollary}\DIFaddbegin \label{cortwo}
    ...
    \begin{itemize}
        ...
        \item or, $x(\Sigma)\cap E(p)\neq \emptyset$
            ...
            and  hence, in any case,  $x(\Sigma) \cap \Omega(p,\epsilon) \neq \emptyset$, ($x^{-1}(\Omega(p,\epsilon))\neq \emptyset$).
\end{corollary}
\DIFdelbegin \DIFdel{In conclusion, in this first result, we have seen that when $x: \Sigma^m \to \mathbb{S}^n(1)$ is  compact and minimal, then }\emph{\DIFdel{always}} %DIFAUXCMD
...
}\DIFdelend %DIF > In conclusion, in this first result, we have seen that when $x: \Sigma^m \to \mathbb{S}^n(1)$ is  compact and minimal, then \emph{always} there are points of $x(\Sigma)$ included in any equatorial strip, some in one part of the equator, some in the other part.

%%%%%%%%%%%%%%%%%%
\section{Main result}
%%%%%%%%%%%%%%%%%%%%

In the above Section\DIFdelbegin \DIFdel{we have seen }\DIFdelend \DIFaddbegin \DIFadd{, we have proved in Corollary \ref{cortwo} }\DIFaddend that when $x: \Sigma^m \to \mathbb{S}^n(1)$ is  compact and minimal, then \emph{always} there are points of $x(\Sigma)$ included in any equatorial strip, some in one part of the equator, some in the other part. The question now is to estimate the \lq\lq amount" of such points in $x(\Sigma)$ which are included in these equatorial strips. 
...
\DIFdelbegin \DIFdel{These ideas are stated in the following results:
...
\end{aligned}
\end{equation}
Then, since $\Delta^\Sigma \cos(r_p(q))=-m\cos(r_p(q))$, and integrating equation \eqref{eqOne} along $\Sigma$, we have, using that $\Sigma$ is compact\DIFaddbegin \DIFadd{, that $\cos\circ r_p$ is smooth in $\Sigma$ }\DIFaddend and the divergence theorem:
...
$$
where $\bar\epsilon=\sin\epsilon \in (0,1)$. Hence, for each \DIFdelbegin \DIFdel{$p$ and $\epsilon$}\DIFdelend \DIFaddbegin \DIFadd{$p \in \ese^n(1)$ and $\epsilon \in (0,\frac{\pi}{2}]$}\DIFaddend ,
$$
\lim_{m\to\infty}\frac{{\rm vol}\left(x^{-1}(\Omega(p,\epsilon))\right)}{{\rm vol}(\Sigma)}=1
$$
\end{theorem}
...
\end{equation}
where  $\widetilde{x}: \mathbb{S}^m(1) \rightarrow \mathbb{S}^n(1)$ is a totally geodesic immersion of $\ese^m(1)$ in $\ese^n(1)$, $r_p$ is the extrinsic distance function to $p$ given \DIFdelbegin \DIFdel{bay }\DIFdelend \DIFaddbegin \DIFadd{by the immersion }\DIFaddend $x$, and $\widetilde{r}_p$ is the extinsic distance to $p$ given by \DIFaddbegin \DIFadd{the totally geodesic immersion }\DIFaddend $\widetilde{x}$. As a consequence, we have the following inequality,  for a given $t>0$:
...
$$
Now, if we consider the totally geodesic submanifold $\widetilde{x}: \mathbb{S}^m(1) \rightarrow \mathbb{S}^n(1)$, \DIFdelbegin \DIFdel{similarly to \eqref{eq:longa}using }\DIFdelend \DIFaddbegin \DIFadd{we conclude the following equality, (in an analogous way  to \eqref{eq:longa}, but using now }\DIFaddend that $\Vert\nabla^{\mathbb{S}^m(1)} \widetilde{r}_p\Vert=1$, because $\widetilde{x}$ is a totally geodesic immersion\DIFdelbegin \DIFdel{, we conclude the following equality
}\DIFdelend \DIFaddbegin \DIFadd{), 
}\DIFaddend \begin{equation}\label{eqcinc}
...
%DIFDELCMD < \Div^\Sigma\left(\cos^{2k+1} (r_p(q))\,\nabla^\Sigma \cos(r_p(q)\right)=&\cos^{2k+1}(r_p(q))\Delta^\Sigma\cos(r_p(q)\\&+(2k+1) \cos^{2k}(r_p(q))\Vert \nabla^\Sigma \cos(r_p(q))\Vert^2\\
%DIFDELCMD < =&-m\cos^{2k+2}(r_p(q))\\&+(2k+1) \cos^{2k}(r_p(q))\Vert \nabla^\Sigma \cos(r_p(q))\Vert^2.
...
\bibitem[T]{Tak} \DIFadd{T. Takahasi, 
}\newblock{\em Minimal immersions of Riemannian manifolds}\DIFadd{,
}\newblock \DIFadd{J. Math. Soc. Japan, }\textbf{\DIFadd{18}}\DIFadd{, n. 4, (1966), 380--385
}\DIFaddend 
