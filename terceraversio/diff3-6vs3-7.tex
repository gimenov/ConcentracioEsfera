...
\title[Fat equator effect and minimality]{\DIFdelbegin %DIFDELCMD < \lq\lq %%%
\DIFdelend Fat equator effect \DIFdelbegin \DIFdel{" }\DIFdelend and Minimality in immersions and submersions of the Sphere}
...
%Abstract
%%%%%%%%%%%%%%%%%%%%
\begin{abstract}\textcolor{red}{V3\DIFdelbegin \DIFdel{.6}\DIFdelend \DIFaddbegin \DIFadd{.7}\DIFaddend }
...
\DIFaddbegin \DIFadd{There is concentration of measure in the dual geometric setting with respect the isometric immersions given by the Riemmanian submersions.  A compact manifold with admits a Riemannian submersion onto the sphere with minimal fibers accumulates almost the whole measure on the  points which project to the strips of any width surounding the equator (see Theorem \ref{TeoSub}).
}

\DIFaddend In the lecture notes by V. Milman and G. Schechtman \cite{MS} we can find the following theorem:
...
\DIFdelendFL \centering
    ...
    \caption{The $\epsilon$-strip of $\mathbb{S}^2(1)$ is the $\epsilon$-fattening of the equator of $\mathbb{S}^2(1)$}\DIFaddbeginFL \label{fig:e-strip}
...
\begin{equation}
\DIFadd{1\geq \frac{{\rm vol}\left(\pi^{-1}(\Omega(p,\epsilon))\right)}{{\rm vol}(\Sigma)}=\frac{{\rm vol}\left(\Omega(p,\epsilon)\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}\geq  1-\sqrt{\frac{\pi}{2}}e^{-\epsilon^2\frac{n-1}{2}}.
}\end{equation}
\DIFadd{Hence, for each  $\epsilon$,
}\begin{equation}
...
\DIFaddend \begin{remark}\label{remarkone}
...
\item%DIFAUXCMD
\DIFdel{For each fixed $m\in \mathbb{N}_{>0}$ in Theorem \ref{mainInt} , the best choice for $\delta$  in inequality \eqref{eq4}, (in the sense that maximizes the lower bound $1- \sqrt{\frac{1}{1-2\delta}}e^{-\delta\bar\epsilon^2(m+1)}$), is attained when
}\begin{displaymath} 
\DIFdel{\delta=\frac{1}{2}\left(1-\frac{1}{ \bar \epsilon^2 (m+1)}\right),
 }\end{displaymath}%DIFAUXCMD
%DIFDELCMD < \noindent %%%
\DIFdel{In this case, inequality  \eqref{eq4} becomes
}\begin{displaymath}
...
%DIFDELCMD < %%%
\DIFdel{We are going to finish this Introduction presenting a concentration of measure result of the fiber bundle projecting on the strip $\Omega(p, \epsilon)$ of a Riemannian submersion $\pi: \Sigma \rightarrow \ese^n(1)$ when this submersion is minimal, namely, when the (compact) fibers $\pi^{-1}(q) \subseteq \Sigma$ with $q \in \ese^n(1)$ are minimally immersed in $\Sigma$. This result, which can be considered a dual property of the extrinsic equatorial concentration of measure given by Theorem \ref{mainInt}, comes from both the structural duality of the equations that govern the geometry of the submersion with respect to the corresponding equations of the immersions and from the fact that the projection $\pi$ commutes with the Laplacians $\Delta^\Sigma$ and $\Delta^{\ese^n(1)}$ when the fibers of $\pi: \Sigma \rightarrow \ese^n(1)$ are minimal submanifolds of $\Sigma$.
}%DIFDELCMD < 

%DIFDELCMD < \begin{theorem}\label{TeoSub}
%DIFDELCMD < %%%
\DIFdel{Let  $\pi: \Sigma^m \to \mathbb{S}^n(1)$ be a Riemannian submersion with minimal fibers  of the closed $m$-dimensional manifold $\Sigma$ on to the sphere  $\mathbb{S}^n(1)$. Then, for any point $p \in \mathbb{S}^n(1)$ , any  $\epsilon \in (0,\pi/2)$ and any $\delta\in [0,\frac{1}{2})$ we have 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
\DIFdel{1\geq \frac{{\rm vol}\left(\pi^{-1}(\Omega(p,\epsilon))\right)}{{\rm vol}(\Sigma)}\geq  1- \sqrt{\frac{1}{1-2\delta}}e^{-\delta\bar\epsilon^2(n+1)}
}\end{displaymath}%DIFAUXCMD
\DIFdel{where $\bar\epsilon=\sin\epsilon$. Hence, for each  $\epsilon$,
}\begin{displaymath}
\DIFdel{\lim_{n\to\infty}\frac{{\rm vol}\left(\pi^{-1}(\Omega(p,\epsilon))\right)}{{\rm vol}(\Sigma)}=1.
}\end{displaymath}%DIFAUXCMD
...
$$
(p,q)\mapsto \pi_\delta(p,q)=p \,\,\forall \delta \in \erre\DIFaddbegin \DIFadd{_{>0}
}\DIFaddend $$
...
$$\lambda_1(\Sigma_\delta)\DIFdelbegin \DIFdel{=}\DIFdelend \DIFaddbegin \DIFadd{\leq }\DIFaddend {\rm min}\{\lambda_1(\mathbb{S}^n(1),g_{\rm can}),\lambda_1(\mathbb{S}^1,\delta^2d\theta^2)\}={\rm min}\{n, \frac{1}{\delta^2}\}$$
...
%DIFDELCMD < %%%
\DIFdel{This proof }\DIFdelend \DIFaddbegin \DIFadd{The proof of Theorem \ref{twopiecegeneralsub} }\DIFaddend follows the lines of the proof of Theorem \ref{twopiecegeneral},  when we consider an isometric immersion \DIFdelbegin \DIFdel{. Let $K$ be a connected component of $\Sigma$. Since $\Sigma$ is parabolic, $K$ is a parabolic manifold as well.
Now, given $p\in \mathbb{S}^n(1)$, the }\emph{\DIFdel{lifted distance function}} %DIFAUXCMD
\DIFdel{with respect to $p$ is the function given by
}\begin{displaymath}
\DIFdel{\begin{aligned}
r_p:K \to \mathbb{R}, \quad q\mapsto r_p(q):&={\rm dist}^{\mathbb{S}^n(1)}(p,\pi(q))\\&=\angle{(p,\pi(q))}=\arccos(p\cdot \pi(q))
\end{aligned}
}\end{displaymath}%DIFAUXCMD
\DIFdel{so, given $q \in K$, with $\pi(q)=(x_1(\pi(q)),...,x_{n+1}(\pi(q))) \in \mathbb{S}^n(1)$, where $x=(x_1,...,x_{n+1})$ is the immersion of $\ese^n(1)$ in $\erre^{n+1}$ we have that
}\begin{displaymath}
\DIFdel{\cos(r_p(q))=p\cdot \pi(q) = p_1\pi^1(q)+\cdots+p_{n+1}\pi^{n+1}(q)
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD <  %%%
\DIFdel{and hence, $\cos\circ r_p$ is smooth on $K$. 
 }%DIFDELCMD < 

%DIFDELCMD <  %%%
\DIFdel{We have, as before, the following
 }%DIFDELCMD < 

%DIFDELCMD <  \begin{lemma}\label{coslema2}
%DIFDELCMD < %%%
\DIFdel{Let $\pi:K^m \to \mathbb{S}^n(1)$ be a complete and minimal Riemannian submersion, and let $p$ be any point of }\DIFdelend \DIFaddbegin \DIFadd{but now using the function $\cos(r_p)$ with $r_p:=d_p \circ \pi$ and $d_p$ the geodesic distance in }\DIFaddend $\mathbb{S}^n(1)$ \DIFdelbegin \DIFdel{. Then, for all $q \in K$,
 }\begin{displaymath}\DIFdel{%DIFDELCMD < \label{cos2}%%%
%DIF < \begin{aligned}
...
%DIFDELCMD < %%%
\DIFdel{As }\DIFdelend \DIFaddbegin \DIFadd{to $p\in \mathbb{S}^n$ . Taking into account that since }\DIFaddend the fibers of $\pi$ are minimal submanifolds, then, (see \cite{W}, \cite{GP})
$$\Delta\DIFdelbegin \DIFdel{^K}\DIFdelend \DIFaddbegin \DIFadd{^\Sigma}\DIFaddend (\DIFdelbegin \DIFdel{x_i}\DIFdelend \DIFaddbegin \DIFadd{f}\DIFaddend \circ \pi)(q)=\bigg(\big(\Delta^{\ese^n(1)}\DIFdelbegin \DIFdel{x_i}\DIFdelend \DIFaddbegin \DIFadd{f}\DIFaddend \big)\circ\pi\bigg)(q)\DIFdelbegin \DIFdel{\,\,\forall q \in K}\DIFdelend $$
  \DIFdelbegin \DIFdel{Applying Takahasi's Lemma, now on the totally geodesic immersion $x: \ese^n(1) \rightarrow \ese^n(1)$ we have that 
  }\begin{displaymath}\DIFdel{\Delta^K (x_i\circ \pi)=\big(\Delta^{\ese^n(1)}x_i\big)\circ\pi=-n x_i\circ\pi=-n\pi^i\,\,\forall i=1,...,n+1}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{for any smooth function $f:\mathbb{S}^n\to \mathbb{R}$. Choosing $f$ as the geodesic distance $d_p$ to $p\in \mathbb{S}^n(1)$ we can state, see Theorem 27 of \mbox{%DIFAUXCMD
\cite{Petersen} }\hskip0pt%DIFAUXCMD
for instance, 
  }$$
  \DIFadd{\Delta^\Sigma r_p=\Delta^\Sigma (d_p\circ \pi)=\Delta^{\ese^n(1)}d_p\circ \pi=(n-1)\frac{\cos(d_p)}{\sin(d_p)}\circ \pi=(n-1)\frac{\cos(r_p)}{\sin(r_p)}
  }$$\DIFaddend 
  \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \noindent   %%%
\DIFdel{so we conclude 
}\begin{displaymath}
\DIFdel{\begin{aligned}
...
\DIFdelend \DIFaddbegin \DIFadd{Hence for any smooth function $F:\mathbb {R}\to \mathbb{R}$,
  }$$
\DIFadd{\Delta^\Sigma F(r_p)=}{\DIFadd{\rm div }}\DIFadd{(F'(r_p)\nabla^\Sigma r_p)=F''(r_p)\Vert \nabla^\Sigma r_p\Vert^2+ (n-1)\frac{\cos(r_p)}{\sin(r_p)} F'(r_p).
}$$\DIFaddend 
  \DIFdelbegin %DIFDELCMD < \end{proof}
%DIFDELCMD <  %%%
\DIFdel{Now, the proof follows as in Theorem \ref{twopiecegeneral}
}%DIFDELCMD < \end{proof}
%DIFDELCMD < %%%
\DIFdelend 

\DIFdelbegin \DIFdel{Note that, if $\Sigma$ is compact, then it is parabolic, so we can apply  the above Theorem \ref{twopiecegeneralsub} to conclude the following corollary:
}%DIFDELCMD < \begin{corollary}\label{cortwo2}
%DIFDELCMD < 	%%%
\DIFdel{Let  $\pi: \Sigma^m \to \mathbb{S}^n(1)$ be a Riemannian submersion of a compact  $m$-dimensional manifold $\Sigma$ in the sphere  $\mathbb{S}^n(1)$, 
  and let $p$ be any point of $\mathbb{S}^n(1)$.  Then
    }%DIFDELCMD < \begin{itemize}
\begin{itemize}%DIFAUXCMD
%DIFDELCMD <         \item %%%
\item%DIFAUXCMD
\DIFdel{Either $\pi(\Sigma)\subseteq E(p)$,
  }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{or, $\pi(\Sigma)\cap E(p)\neq \emptyset$
            }
\end{itemize}%DIFAUXCMD
%DIFDELCMD < \end{itemize}
%DIFDELCMD <             %%%
\DIFdel{and  hence, in any case,  $\pi(\Sigma) \cap \Omega(p,\epsilon) \neq \emptyset$, (i.e. $\pi^{-1}(\Omega(p,\epsilon))\neq \emptyset$).
}%DIFDELCMD < \end{corollary}
%DIFDELCMD < \begin{remark}
%DIFDELCMD < %%%
\DIFdel{Note that in all the above results, we can replace the hypothesis of parabolicity for the weaker assumption of stochastic completeness. 
}%DIFDELCMD < \end{remark}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{By expressing $\nabla^\Sigma r$ in terms an orthonormal basis of the horizontal distribution and then projecting on the base space using the fact that $r_p=d_p\circ \pi$ we conclude that $\Vert \nabla^\Sigma r_p\Vert^2=1$ and hence 
}\begin{equation}\DIFadd{\label{eq:lapsub}
    \Delta^\Sigma F(r_p)=F''(r_p)+(n-1) \frac{\cos(r_p)}{\sin(r_p)} F'(r_p).
}\end{equation}
\DIFadd{In particular
}$$
\DIFadd{\Delta^\Sigma \cos(r_p)=-n \cos(r_p),
}$$
\DIFadd{what is everything needed for the proof of the above theorem. 
}\DIFaddend %%%%%%%%%%%%%%%%%%
...
\end{equation}
where $\widetilde{x}: \mathbb{S}^m(1) \rightarrow \mathbb{S}^n(1)$ is a totally geodesic immersion of $\ese^m(1)$ in $\ese^n(1)$, $r_p$ is the extrinsic distance function to $p$ given by the immersion $x$, and $\widetilde{r}_p$ is the \DIFdelbegin \DIFdel{extinsic }\DIFdelend \DIFaddbegin \DIFadd{extrinsic }\DIFaddend distance to $p$ \DIFaddbegin \DIFadd{on $\ese^n(1)$ }\DIFaddend given by the totally geodesic immersion $\widetilde{x}$. As a consequence, we have the following inequality,  for a given $t>0$:

\begin{equation}\label{lemmaformula2}
%\begin{aligned}
\int_{\Sigma} e^{t \cos^2r_p}  \leq\frac{\int_{\ese^m(1)} e^{t \cos^2\widetilde{r}_p}}{{\rm vol}(\mathbb{S}^m(1))} {\rm vol}\DIFdelbegin \DIFdel{.}\DIFdelend (\Sigma)
%\end{aligned}
\end{equation}
\end{lemma}
\begin{proof}
...
\begin{equation}
1\geq \frac{{\rm vol}\left(\pi^{-1}(\Omega(p,\epsilon))\right)}{{\rm vol}(\Sigma)}\DIFaddbegin \DIFadd{=\frac{{\rm vol}\left(\Omega(p,\epsilon)\right)}{{\rm vol}\left(\mathbb{S}^n(1)\right)}}\DIFaddend \geq  1-\DIFdelbegin \DIFdel{\sqrt{\frac{1}{1-2\delta}}}\DIFdelend \DIFaddbegin \DIFadd{\sqrt{\frac{\pi}{2}}}\DIFaddend e\DIFdelbegin \DIFdel{^{-\delta\bar\epsilon^2(n+1)}
}\DIFdelend \DIFaddbegin \DIFadd{^{-\epsilon^2\frac{n-1}{2}}.
}\DIFaddend \end{equation}
\DIFdelbegin \DIFdel{where $\bar\epsilon=\sin\epsilon$. }\DIFdelend Hence, for each  $\epsilon$,
\begin{equation}
\lim_{n\to\infty}\frac{{\rm vol}\left(\pi^{-1}(\Omega(p,\epsilon))\right)}{{\rm vol}(\Sigma)}=1.
\end{equation}
\end{theorem}
\begin{proof}
\DIFdelbegin \DIFdel{As the Riemannian submersion $\pi:\Sigma \rightarrow \ese^(1)$ is minimal, namely, the fibers $\pi^{-1}(p)$ are minimal submanifolds of the total space $\Sigma$, then we know that the Laplacian of any basic function commutes with the submersion , }\emph{\DIFdel{i.e.}}%DIFAUXCMD
\DIFdel{, if $\pi^*$ is the pullback of $\pi$, acting on functions, then
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
\DIFdel{\Delta^\Sigma \pi^* f=\pi^*\Delta^{\mathbb{S}^n}f
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{Since $\pi:\Sigma \rightarrow \ese^n(1)$ is a Riemmanian submersion we have that
}$$
\DIFadd{\Vert \nabla r_p(x)\Vert =\Vert \nabla \left(d_p\circ \pi\right)\Vert=1, \quad x\in \Sigma-\{\pi^{-1}(p)\}\cup \{\pi^{-1}(-p) \}
}$$\DIFaddend 
\DIFdelbegin %DIFDELCMD < \noindent %%%
\DIFdelend \DIFaddbegin \DIFadd{where  $d_p$ is the distance function on $\mathbb{S}^n(1)$ to $p$. 
Let us denote by $D_R$ the sublevel sets of $r_p$, }\DIFaddend namely
\DIFdelbegin \begin{displaymath}
\DIFdel{\Delta^\Sigma( f\circ \pi)=(\Delta^{\mathbb{S}^n}f) \circ \pi
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin $$
\DIFadd{D_R:=\{x\in \Sigma\, : \, r_p(x)<R\}=\pi^{-1}(B_R(p))
}$$\DIFaddend 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \noindent %%%
\DIFdel{Using this property we have proved in Lemma \ref{coslema2} that
}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{distance}%%%
\Delta^\Sigma \cos(r_p(q))=-n\cos(r_p(q))\,\,\forall q \in \Sigma
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < \noindent  %%%
\DIFdel{where  , given $p \in \ese^n(1)$, we define the }%DIFDELCMD < {\em %%%
\DIFdel{lifted}%DIFDELCMD < } %%%
\DIFdel{distance function from }\DIFdelend \DIFaddbegin \DIFadd{where $B_R(p)$ is the geodesic ball of radius $R$ of $\mathbb{S}^n(1)$ centered at }\DIFaddend $p$\DIFdelbegin \DIFdel{as $r_p(q)={\rm dist}^{\ese^n(1)}(p,\pi(q))$. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{On the other hand it is an straightforward computation to prove that
}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{norm}%%%
\Vert \nabla^\Sigma r_p\Vert \leq 1
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{. By the coarea formula the function $t\mapsto U(t)={\rm vol}(D_t)$ has derivative
}$$
\DIFadd{U'(t)=\int_{\partial D_t}\frac{1}{\Vert \nabla r_p\Vert}=}{\DIFadd{\rm vol}}\DIFadd{(\partial D_t)
}$$\DIFaddend 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Now, we proceed exactly as in the proofs of Theorems \ref{extrinsicconc1} and \ref{extconc2}, but fixing the dimension $m=n$: using equations \eqref{distance} and \eqref{norm}, we conclude, applying divergence theorem in the compact without boundary Riemannian manifold $\Sigma$, that
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}\DIFdel{%DIFDELCMD < \label{eq:longabis1}%%%
\int_\Sigma \cos^2(r_p) \leq \frac{1}{n+1} }{\DIFdel{\rm vol}}\DIFdel{(\Sigma)
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{for any $t\in (0,\pi)$. When we consider the function $E:\Sigma\to \mathbb{R}$ given by
}$$
\DIFadd{q\in \Sigma\mapsto E(q)=\int_0^{r_p(q)}\frac{V(t)}{V'(t)}dt
}$$\DIFaddend 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Now, if we consider the totally geodesic Riemannian submersion given by the identity map $\widetilde{\pi}: \mathbb{S}^n(1) \rightarrow \mathbb{S}^n(1)$, we conclude the following equality, (in an analogous way  to \eqref{eq:longabis1}, but using now that $\Vert\nabla^{\mathbb{S}^n(1)} \widetilde{r}_p\Vert=1$, because $\widetilde{\pi}$ has totally geodesic fibers), 
}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{eqcinc}%%%
\int_{\mathbb{S}^n(1)} \cos^2(\widetilde{r}_p) = \frac{1}{n+1} }{\DIFdel{\rm vol}}\DIFdel{(\mathbb{S}^n(1)).
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Hence,  we have 
}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{eqsisbis}%%%
\int_\Sigma \cos^2(r_p) \leq \frac{\int_{\mathbb{S}^n(1)} \cos^2(\widetilde{r}_p)}{{\rm vol}(\mathbb{S}^n(1))} }{\DIFdel{\rm vol}}\DIFdel{(\Sigma)
}\end{displaymath}%DIFAUXCMD
\DIFdel{and we have proved that the inequality (\ref{lemmaformula1bis}) is true for $k=1$. }%DIFDELCMD < \noindent %%%
\DIFdel{Then, we have the following 
}%DIFDELCMD < 

%DIFDELCMD < \begin{lemma}\label{keylemasub}
%DIFDELCMD < %%%
\DIFdel{Let  $\pi: \Sigma^m \to \mathbb{S}^n(1)$ be a Riemannian submersion of the compact $m$-dimensional manifold $\Sigma$ on the sphere  $\mathbb{S}^n(1)$ and let $p\in \mathbb{S}^n(1)$. Then
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}\DIFdel{%DIFDELCMD < \label{lemmaformula1bis}%%%
\int_\Sigma \cos^{2k}(r_p) \leq \frac{\int_{\mathbb{S}^n(1)} \cos^{2k}(\widetilde{r}_p)}{{\rm vol}(\mathbb{S}^n(1))} }{\DIFdel{\rm vol}}\DIFdel{(\Sigma)
}\end{displaymath}%DIFAUXCMD
\DIFdelend where \DIFdelbegin \DIFdel{$\widetilde{\pi}: \mathbb{S}^n(1) \rightarrow \mathbb{S}^n(1)$ is the totally geodesic Riemannian submersion of $\ese^n(1)$ in $\ese^n(1)$ given by identity map $\widetilde{\pi}=id_{\ese^n(1)}$, $r_p$ is the lifted distance function to $p$ given by
the submersion $\pi$, and $\widetilde{r}_p$ is the lifted distance to $p$ given by the totally geodesic submersion $\widetilde{\pi}$. As a consequence, we have the following inequality,  for a given $t>0$:
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}\DIFdel{%DIFDELCMD < \label{lemmaformula2bis}%%%
%DIF < \begin{aligned}
\int_{\Sigma} e^{t \cos^2r_p}  \leq\frac{\int_{\ese^n(1)} e^{t \cos^2\widetilde{r}_p}}{{\rm vol}(\mathbb{S}^n(1))} }{\DIFdel{\rm vol}}\DIFdel{(\Sigma)
%DIF < \end{aligned}
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{$V(t)={\rm vol}(B_t(p))$, and $V'(t)=\frac{d}{dt}V(t)={\rm vol }(\partial B_t(p))$. By using \eqref{eq:lapsub} can be proved that
}$$
\DIFadd{\Delta^{\Sigma}E=1.
}$$\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdel{Now,
it is easy to check that, for all $q \in\Sigma-\pi^{-1}(\Omega(p,\epsilon))$, 
}\begin{displaymath} \DIFdel{\bar\epsilon^2 \leq \cos^2(r_p(q))}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{Hence by using the divergence theorem we can deduce that
}$$
...
\DIFdelbegin %DIFDELCMD < \noindent %%%
\DIFdel{so we can conclude, in the same way than in Theorem \ref{extconc2}, that , for every $t >0$,
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}\DIFdel{%DIFDELCMD < \label{preskauxlastabis}%%%
...
\DIFdel{Now we can prove the following result, in the same way we did in Lemma \ref{lastalemma}
}%DIFDELCMD < \begin{lemma}\label{lastalemmabis} %%%
\DIFdel{For any $0<t<\frac{n+1}{2}$,
}\DIFdelend \DIFaddbegin \DIFadd{Thence,
}$$
\DIFadd{\frac{d}{dt}\left(\frac{U(t)}{V(t)}\right)=0
}$$
\DIFadd{for all $t\in (0,\pi)$. This implies that for any $R\in(0,\pi)$
}\DIFaddend $$
\DIFdelbegin \DIFdel{\frac{\int_{\mathbb{S}^n(1)}e^{t \cos^2\widetilde{r}_p}}{{\rm vol}(\mathbb{S}^n(1))}\leq \sqrt{\frac{n+1}{n+1-2t}}}\DIFdelend \DIFaddbegin \DIFadd{\frac{{\rm vol}(\pi^{-1}(B_R(p)))}{{\rm vol}(B_R(p))}=\lim_{R\to \pi}\frac{{\rm vol}(\pi^{-1}(B_R(p)))}{{\rm vol}(B_R(p))}=\frac{{\rm vol}(\Sigma)}{{\rm vol}(\mathbb{S}^n(1))}}\DIFaddend .
$$
\DIFdelbegin %DIFDELCMD < \end{lemma}
%DIFDELCMD < \begin{proof}
%DIFDELCMD <     %%%
\DIFdel{Let us define the function 
    }\begin{displaymath}
\DIFdel{s\mapsto F(s):= \frac{\int_{\mathbb{S}^n(1)}e^{s \cos^2\widetilde{r}_p}}{{\rm vol}(\mathbb{S}^n(1))}.
    }\end{displaymath}%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{By the dominated convergence theorem we know that $F(0)=1$ and
     }%DIFDELCMD < 

%DIFDELCMD <     %%%
\begin{displaymath}
\DIFdel{F'(s)=\frac{\int_{\mathbb{S}^n(1)}\cos^2\widetilde{r}_pe^{s \cos^2\widetilde{r}_p}}{{\rm vol}(\mathbb{S}^n(1))}
    }\end{displaymath}%DIFAUXCMD
%DIFDELCMD <  \noindent %%%
\DIFdel{By using lemma \ref{coslema2}, the divergence theorem, and that since the submersion given by the identity map $\widetilde{\pi}: \mathbb{S}^n(1) \rightarrow \mathbb{S}^n(1)$ is totally geodesic, so we have  $\Vert \nabla\widetilde{r}_p\Vert^2=1$, we compute as in Lema \ref{lastalemma} to obtain
 }\DIFdelend \DIFaddbegin \DIFadd{Finally the theorem follows by using
}\DIFaddend $$
%DIF < \begin{aligned}
\DIFdelbegin \DIFdel{nF'(s)\leq  F(s)-F'(s)+2sF'(s).
  %DIF <   \end{aligned}
...
\DIFaddend $$
\DIFdelbegin %DIFDELCMD < \noindent %%%
\DIFdel{and we proceed as in the proof of Lemma \ref{lastalemma}.
}%DIFDELCMD < \end{proof}
%DIFDELCMD < %%%
\DIFdel{Taking now $t=\delta(n+1)$ with $0\leq \delta<\frac{1}{2}$, the above lemma and inequality \eqref{preskauxlastabis} we conclude that
}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{lasta}%%%
     1-\frac{{\rm vol}(\pi^{-1}(\Omega(p,\epsilon))}{{\rm vol}(\Sigma)}\leq e^{-\delta\bar\epsilon^2(n+1)}\sqrt{\frac{1}{1-2\delta}}.
}\end{displaymath}%DIFAUXCMD
\DIFdel{and the theorem is proved.
}\DIFdelend \DIFaddbegin \DIFadd{and the Theorem follows by using Theorem \ref{equator}.
}\DIFaddend 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \section{\DIFdel{Appendix: Proof of theorem \ref{teo:mitjana}}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section*{\DIFadd{Appendix I: Proof of theorem \ref{teo:mitjana}}}\DIFaddend \label{appendix}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\DIFaddbegin \bibitem{Petersen}
\DIFadd{P. Petersen.
}\newblock {\em \DIFadd{Riemannian geometry}}\DIFadd{, volume 171 of }{\em \DIFadd{Graduate Texts in
  Mathematics}}\DIFadd{.
}\newblock \DIFadd{Springer-Verlag, New York, 1998.
}\DIFaddend 
...
%Abstract
%%%%%%%%%%%%%%%%%%%%
\begin{abstract}\textcolor{red}{V3.7}
Inspired by the equatorial concentration of measure phenomenon in the sphere, a result which is deduced from the general, (and intrinsic), concentration of measure in $\ese^n(1)$, we describe in this paper an extrinsic equatorial concentration of measure satisfied by the closed, (compact without boundary), isometric and minimal immersions $x:\Sigma^m \rightarrow \ese^n(1)$, ($m \leq n$), and by the minimal Riemannian submersions $\pi: \Sigma^m \rightarrow \ese^n(1)$, ($m \geq n$).\end{abstract}
...
There is concentration of measure in the dual geometric setting with respect the isometric immersions given by the Riemmanian submersions.  A compact manifold with admits a Riemannian submersion onto the sphere with minimal fibers accumulates almost the whole measure on the  points which project to the strips of any width surounding the equator (see Theorem \ref{TeoSub}).
...
$$
All these Riemannian submersions are minimal, indeed totally geodesic ones, and, as $\Sigma_\delta:=(\mathbb{S}^n(1),g_{\rm can}) \times (\mathbb{S}^1,\delta^2d\theta^2)$  for any $\delta \in \erre$ we have that
$$\lambda_1(\Sigma_\delta)\leq {\rm min}\{\lambda_1(\mathbb{S}^n(1),g_{\rm can}),\lambda_1(\mathbb{S}^1,\delta^2d\theta^2)\}={\rm min}\{n, \frac{1}{\delta^2}\}$$
\noindent so, for any $a\in (0,n]$ there is a value of $\delta$ such that
$$
\lambda_1(\Sigma_\delta)<a.
$$
...
Let $\pi:\Sigma^m \to \mathbb{S}^n(1)$ be a complete, minimal and parabolic Riemannian submersion, and let $p$ be any point of $\mathbb{S}^n(1)$.  Then
    \begin{itemize}
        ...
        \item or, $\pi(\Sigma)\cap E(p)\neq \emptyset$
            ...
            and  hence, in any case,  $\pi(\Sigma) \cap \Omega(p,\epsilon) \neq \emptyset$, (i.e. $\pi^{-1}(\Omega(p,\epsilon))\neq \emptyset$).
\end{theorem}
The proof of Theorem \ref{twopiecegeneralsub} follows the lines of the proof of Theorem \ref{twopiecegeneral},  when we consider an isometric immersion but now using the function $\cos(r_p)$ with $r_p:=d_p \circ \mathbb{S}^n(1)$  and $d_p$ the geodesic distance in $\mathbb{S}^n(1)$ to $p\in \mathbb{S}^n$ . Taking into account that since the fibers of $\pi$ are minimal submanifolds, then, (see \cite{W}, \cite{GP})
$$\Delta^\Sigma(f\circ \pi)(q)=\bigg(\big(\Delta^{\ese^n(1)}f\big)\circ\pi\bigg)(q)$$
  ...
  $$
  \Delta^\Sigma r_p=\Delta^\Sigma (d_p\circ \pi)=\Delta^{\ese^n(1)}d_p\circ \pi=(n-1)\frac{\cos(d_p)}{\sin(d_p)}\circ \pi=(n-1)\frac{\cos(r_p)}{\sin(r_p)}
  $$
...
Since $r_p=\widetilde r_p\circ \pi$ we can deduce moreover that 
$$
\Vert \nabla r_p(x)\Vert =1, \quad x\in \Sigma-\{\pi^{-1}(p)\}\cup \{\pi^{-1}(-p) \}
$$
Let us denote by $D_R$ the sublevel sets of $r_p$, namely
$$
D_R:=\{x\in \Sigma\, : \, r_p(x)<R\}=\pi^{-1}(B_R(p))
$$
where $B_R(p)$ is the geodesic ball of radius $R$ of $\mathbb{S}^n(1)$ centered at $p$. By the coarea formula the function $t\mapsto U(t)={\rm vol}(D_t)$ has derivatide
$$
U'(t)=\int_{\partial D_t}\frac{1}{\Vert \nabla r_p\Vert}={\rm vol}(\partial D_t)
$$
for any $t\in (0,\pi)$. Let us consider the function $E:\Sigma\to \mathbb{R}$ given by
$$
q\in \Sigma\mapsto E(q)=\int_0^{r_p(q)}\frac{V(t)}{V'(t)}dt
$$
where $V(t)={\rm vol}(B_t(p))$, and $V'(t)=\frac{d}{dt}V(t)={\rm vol }(\partial B_t(p))$. By using \eqref{eq:lapsub} can be proved that
$$
\Delta^{\Sigma}E=1,
$$
...
\end{aligned}
$$
abd tge Theorem follows by using Theorem \ref{equator}.
